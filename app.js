...
// Pas d'exercices de rédaction guidée en 4ème (spécifiques au Brevet 3ème)
const REDACTION_FOR_GEN = {};

/* Fiches consultées — stockées localement */
const REV_VIEWED_KEY = 'auto4e.rev.viewed';
...
/* ==========================================================================
   ...
   ========================================================================== */
const STORE_KEYS = { STUDENT: 'auto4e.student', SESSIONS: 'auto4e.sessions', REPORTS: 'auto4e.reports' };
...
const EXPORT_APP = 'automatismes-4eme';
const EXPORT_REMINDER_KEY = 'auto4e.export.lastReminder';
const EXPORT_LAST_EXPORT_KEY = 'auto4e.export.lastExportAt';
const EXPORT_KEYS = [
  'auto4e.student',
  'auto4e.history',
  'auto4e.parcours',
...
function exportProgression() {
  ...
  const fileName = `auto4e-${safe(st.prenom) || 'anonyme'}-${safe(st.classe) || 'classe'}-${datePart}.json`;
  ...
  reader.onload = () => {
    try {
      ...
      Object.entries(payload.data).forEach(([k, v]) => {
        ...
        restoredKeys++;
        if (k === 'auto4e.history' && Array.isArray(v)) sessionsCount = v.length;
      });
...
/* ==========================================================================
   ...
   ========================================================================== */
const PARCOURS_KEY = 'auto4e.parcours';
...
const DUEL_KEYS = {
  PLAYERS: 'auto4e.duel.players',
  MATCHES: 'auto4e.duel.matches'
};
...
/* ==========================================================================
   ...
   ========================================================================== */
const PWA_TIP_DISMISSED_KEY = 'auto4e.pwa.tip.dismissed';
const PWA_HELP_SHOWN_KEY = 'auto4e.pwa.help.shown';
...
window.addEventListener('appinstalled', () => {
  ...
  if (tip) tip.remove();
  if (!localStorage.getItem('auto4e.pwa.installed')) {
    localStorage.setItem('auto4e.pwa.installed', '1');
...
if (isIOS() && !isStandaloneMode() && !localStorage.getItem(PWA_TIP_DISMISSED_KEY)) {
  const visits = parseInt(localStorage.getItem('auto4e.visits') || '0');
  localStorage.setItem('auto4e.visits', String(visits + 1));
  if (visits >= 1) {
    // 2e visite ou plus
...
if (!getStudent() && !localStorage.getItem('auto4e.welcome-dismissed')) {
...
function showWelcomeBanner() {
  ...
  $('#welcome-dismiss').addEventListener('click', () => {
    localStorage.setItem('auto4e.welcome-dismissed', '1');
    banner.remove();
  });
